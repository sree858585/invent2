            ...
            <header class="modal-header">
                <div>
                    ...
                    <p class="modal-subtitle">
                        Update banner details, linked action, active dates, and optional image.
                    </p>
                </div>
            ...
            <form @submit.prevent="submitBanner">
                <section class="section-card">
                    <div class="section-header">
                        ...
                        <p>Update banner details and click behavior.</p>
                    </div>
                ...
                <section class="section-card">
                    <div class="section-header">
                        <h3>Banner Image</h3>
                        <p>Upload a new image only if you want to replace the current one.</p>
                    </div>

                    <div class="form-group">
                        <label>Banner Image <span class="muted">(optional for edit)</span></label>
                        ...
                        <div class="image-uploader">
                            ...
                            <div class="image-empty" v-else>
                                ...
                                <div class="img-text">
                                    <div class="img-title">Current banner image</div>
                                    <div class="img-sub">Choose a new file only if you want to replace it</div>
                                </div>
                            </div>
...
<script>import apiClient from "@/axios";
    ...
    export default {
        ...
        methods: {
            populateForm() {
                this.form.bannerName = this.banner.bannerName || "";
                this.form.actionType = this.banner.actionType || "";
                this.form.courseSysId = this.banner.courseSysId || "";
                this.form.modalTitle = this.banner.modalTitle || "";
                this.form.modalBodyHtml = this.banner.modalBodyHtml || "";
                this.form.displayOrder = this.banner.displayOrder || 1;
                this.form.active = !!this.banner.active;
                this.form.startDate = this.toInputDate(this.banner.startDate);
                this.form.endDate = this.toInputDate(this.banner.endDate);

                if (this.banner.imageUrl) {
                    this.imagePreviewUrl = this.fullImageUrl(this.banner.imageUrl);
                }
            },
            ...
            fullImageUrl(url) {
                ...
                const cleanedBase = base.endsWith("/api") ? base.replace(/\/api$/, "") : base;
            ...
            clearImage() {
                ...
                this.imagePreviewUrl = this.banner.imageUrl
                    ? this.fullImageUrl(this.banner.imageUrl)
                    : "";
            ...
            async submitBanner() {
                try {
                    ...
                    if (this.form.courseSysId) fd.append("CourseSysId", this.form.courseSysId);
                    if (this.form.modalTitle) fd.append("ModalTitle", this.form.modalTitle);
                    if (this.form.modalBodyHtml) fd.append("ModalBodyHtml", this.form.modalBodyHtml);
                    if (this.form.startDate) fd.append("StartDate", this.form.startDate);
                    if (this.form.endDate) fd.append("EndDate", this.form.endDate);
                    if (this.imageFile) fd.append("File", this.imageFile);

                    await apiClient.put(`/HomeBanner/update/${this.banner.homeBannerSysId}`, fd, {
                        headers: {
