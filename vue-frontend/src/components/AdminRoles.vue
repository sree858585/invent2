<template>
    <div class="admin-roles-container">
        ...
        <table class="modern-table" v-if="users.length > 0">
            ...
            <tbody>
                <tr v-for="user in users" :key="user.userId">
                    ...
                    <td>
                        <button :class="['btn-action', user.isAdmin ? 'remove' : 'add']"
        ...
        <AccessConfirmModal v-if="showConfirmModal"
                            ...
                            :isGranting="!selectedUser?.isAdmin"
                            roleType="Admin"
        ...
        <AccessSuccessModal v-if="showSuccessModal"
                            :user="selectedUser"
                            :action="selectedUser?.isAdmin ? 'removed from' : 'granted'"
                            roleType="Admin"
    ...
    export default {
        ...
        methods: {
            async fetchUsers() {
                ...
                try {
                    const res = await apiClient.get('/RoleManagement/admins', { params });
            ...
            async confirmToggle() {
                ...
                const url = this.selectedUser.isAdmin
                    ? '/RoleManagement/remove-admin'
                    : '/RoleManagement/assign-admin';
                ...
                try {
                    ...
                    console.error('Failed to update admin access:', err);
                    this.showConfirmModal = false;
                }
            },
            changePage(page) {
